...
# Webhook por defecto (usado si la categoría no tiene uno propio)
...
def load_watchlist() -> dict:
    """
    Devuelve un dict: { address: {category_id, category_name, webhook_url, webhook_secret} }
    Solo direcciones activas con categoría activa (o sin categoría).
    """
    with get_db() as conn:
        rows = conn.execute("""
            SELECT
...
def send_webhook(address: str, meta: dict, txid: str, payload: dict) -> bool:
    ...
    if not url:
        log.warning("Sin webhook para categoría '%s', omitiendo notificación",
                    meta["category_name"])
        return False
...
class MempoolWatcher:
    ...
    def _load_and_push(self):
        ...
        if not new_wl:
            log.warning("Sin direcciones activas. Agrega con: manage.py address add")
            return

        if self.ws and self.ws.sock and self.ws.sock.connected:
            if changed:
                log.info("Watchlist actualizada: %d dir(s). Re-registrando…",
                         len(new_wl))
    ...
    def _send_track(self, addresses: list):
        try:
            ...
            log.info("📡 track-addresses → %d dirección(es) en el nodo", len(addresses))
        except Exception as e:
    ...
    def on_open(self, ws):
        log.info("🔗 Conectado al WebSocket de Mempool (%s)", get_ws_url())
        self._load_and_push()
        self._schedule_sync()
